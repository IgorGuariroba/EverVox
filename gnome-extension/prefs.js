// Preferências do EverVox (issue #19, ADR 0004): tela GTK4/Adwaita hospedada
// nas prefs da extensão já existente, aberta pelo app Extensions do GNOME.
// Edita o `config.toml` do Daemon diretamente e gerencia chaves de API no
// GNOME Keyring via Secret Service (libsecret). Ao salvar, chama o método de
// recarga do Daemon — campos quentes (idiomas, Limpeza inteira, Vocabulário,
// terminais conhecidos) aplicam na hora; trocar Engine ou modelo local exige
// reiniciar o Daemon, e a UI avisa e oferece o restart.
//
// Sem a extensão instalada/habilitada, ou sem o Daemon rodando, a CLI +
// `config.toml` seguem como fallback completo (ver `CONTEXT.md`): esta tela
// só facilita a edição, nunca é o único caminho.
//
...
export default class EverVoxPreferences extends ExtensionPreferences {
    fillPreferencesWindow(window) {
        ...
        pagina.add(this._grupoIdiomas(config));
        pagina.add(this._grupoChaves());
        pagina.add(this._grupoEngine(config));
        pagina.add(this._grupoLimpeza(config));
        window.add(pagina);

        const grupoSalvar = new Adw.PreferencesGroup();
        const linhaSalvar = new Adw.ActionRow({title: _('Salvar Preferências')});
        const botaoSalvar = new Gtk.Button({
            label: _('Salvar'),
            valign: Gtk.Align.CENTER,
            css_classes: ['suggested-action'],
        });
        botaoSalvar.connect('clicked', () => this._salvar(window, config));
        linhaSalvar.add_suffix(botaoSalvar);
        grupoSalvar.add(linhaSalvar);
        pagina.add(grupoSalvar);
    }

    _grupoIdiomas(config) {
        const grupo = new Adw.PreferencesGroup({title: _('Idiomas')});

        const entrada = new Adw.EntryRow({title: _('Idioma de entrada')});
        entrada.set_text(config.idioma_entrada);
        entrada.connect('notify::text', () => config.idioma_entrada = entrada.get_text());
        grupo.add(entrada);

        const saida = new Adw.EntryRow({title: _('Idioma de saída')});
    ...
    _grupoChaves() {
        const grupo = new Adw.PreferencesGroup({
            title: _('Chaves de API'),
            description: _('Chaves nunca são exibidas depois de salvas: só o GNOME Keyring as guarda.'),
        });

        for (const provedor of PROVEDORES_DE_CHAVE)
            grupo.add(this._linhaDeChave(provedor));

        return grupo;
    }

    _linhaDeChave(provedor) {
        ...
        const atualizarSubtitulo = () => {
            ...
            linha.set_subtitle(chave ? _('Chave salva ✓') : _('Nenhuma chave salva'));
        };
        atualizarSubtitulo();

        const botaoSubstituir = new Gtk.Button({label: _('Substituir'), valign: Gtk.Align.CENTER});
        botaoSubstituir.connect('clicked', () => this._dialogoDeChave(linha, provedor, atualizarSubtitulo));
        linha.add_suffix(botaoSubstituir);

        const botaoRemover = new Gtk.Button({label: _('Remover'), valign: Gtk.Align.CENTER});
    ...
    _dialogoDeChave(linhaPai, provedor, aoSalvar) {
        const entradaSenha = new Adw.PasswordEntryRow({title: _('Chave de API')});
        const grupo = new Adw.PreferencesGroup();
        grupo.add(entradaSenha);

        const dialogo = new Adw.MessageDialog({
            transient_for: linhaPai.get_root(),
            heading: _('Chave de API — %s').format(provedor),
            extra_child: grupo,
        });
        dialogo.add_response('cancelar', _('Cancelar'));
        dialogo.add_response('salvar', _('Salvar'));
    ...
    _grupoEngine(config) {
        const grupo = new Adw.PreferencesGroup({
            title: _('Engine'),
            description: _('Trocar o Engine ou o modelo local exige reiniciar o Daemon.'),
        });

        const engine = new Adw.ComboRow({
            title: _('Engine de transcrição'),
        ...
        const modelo = new Adw.EntryRow({title: _('Modelo local (whisper.cpp)')});
    ...
    _grupoLimpeza(config) {
        const limpeza = config.limpeza;
        const grupo = new Adw.PreferencesGroup({title: _('Limpeza')});

        const habilitada = new Adw.SwitchRow({title: _('Limpeza ligada'), active: limpeza.habilitada});
        ...
        const modelo = new Adw.EntryRow({title: _('Modelo')});
        ...
        const instrucoes = new Adw.EntryRow({title: _('Instruções da Limpeza')});
        ...
        const vocabulario = new Adw.EntryRow({title: _('Vocabulário (separado por vírgula)')});
        ...
        const pontuacaoFalada = new Adw.SwitchRow({title: _('Pontuação falada'), active: limpeza.pontuacao_falada});
    ...
    _salvar(window, config) {
        salvarConfig(config);
        const resultado = recarregarConfigNoDaemon();

        if (resultado === null) {
            this._avisar(window, _('Daemon não está rodando'),
                _('As mudanças foram salvas no config.toml e serão aplicadas no próximo início do Daemon.'));
            return;
        }
        if (resultado.startsWith('erro:')) {
            this._avisar(window, _('Falha ao recarregar'), resultado);
            return;
        }
        if (resultado === 'restart_necessario') {
            this._confirmarRestart(window);
            return;
        }
        this._avisar(window, _('Preferências salvas'), _('Aplicadas sem precisar reiniciar o Daemon.'));
    }

    _confirmarRestart(window) {
        const dialogo = new Adw.MessageDialog({
            transient_for: window,
            heading: _('Reiniciar o Daemon?'),
            body: _('Trocar o Engine ou o modelo local só vale depois de reiniciar o Daemon.'),
        });
        dialogo.add_response('depois', _('Reiniciar depois'));
        dialogo.add_response('agora', _('Reiniciar agora'));
